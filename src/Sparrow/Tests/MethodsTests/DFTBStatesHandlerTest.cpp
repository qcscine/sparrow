...
#pragma omp critical
  ...
  settingsDFTB0.modifyString(Utils::SettingsNames::loggerVerbosity, Utils::SettingsNames::LogLevels::none);
  ...
  settingsDFTB3.modifyString(Utils::SettingsNames::loggerVerbosity, Utils::SettingsNames::LogLevels::none);
  ...
  underlyingDFTB0_.initializeFromParameterPath(parameters_3ob_2_1);
  underlyingDFTB3_.initializeFromParameterPath(parameters_3ob_2_1);
}
}; // namespace Sparrow
}; // namespace Scine

TEST_F(ADFTBStatesHandlerTest, CanSaveMinimalStateWithDFTB0) {
  underlyingDFTB0_.calculate(Utils::derivativeType::none);
  auto results = interfaceDFTB0_->calculate("");

  interfaceDFTB0_->statesHandler().store(Utils::StateSize::minimal);
  auto savedState = interfaceDFTB0_->statesHandler().popNewestState();
...
TEST_F(ADFTBStatesHandlerTest, CanSaveRegularlySizedStateWithDFTB0) {
  underlyingDFTB0_.calculate(Utils::derivativeType::none);
  auto results = interfaceDFTB0_->calculate("");

  interfaceDFTB0_->statesHandler().store(Utils::StateSize::regular);
  auto savedState = interfaceDFTB0_->statesHandler().popNewestState();
  auto& stateFock = savedState->getMatrixState("Fock Matrix");
  auto& actualFock = underlyingDFTB0_.getFockMatrix().restrictedMatrix();
  auto& stateDensity = savedState->getMatrixState("Density Matrix");
  auto& actualDensity = underlyingDFTB0_.getDensityMatrix().restrictedMatrix();
...
TEST_F(ADFTBStatesHandlerTest, LoadsStateCorrectlyWithDFTB0) {
  ...
  auto densityMatrix = underlyingDFTB0_.getDensityMatrix();

  SparrowState state(Utils::StateSize::minimal, dynamic_cast<GenericMethodWrapper&>(*interfaceDFTB0_));
  state.generateDensityMatrixState(densityMatrix, false);

  interfaceDFTB0_->statesHandler().load(std::make_shared<SparrowState>(std::move(state)));
  interfaceDFTB0_->statesHandler().store(Utils::StateSize::minimal);

  auto loadedDensityMatrix = interfaceDFTB0_->statesHandler().popNewestState()->getMatrixState("Density Matrix");
...
TEST_F(ADFTBStatesHandlerTest, CanSaveMinimalStateWithDFTB3) {
  underlyingDFTB3_.calculate(Utils::derivativeType::none);
  auto results = interfaceDFTB3_->calculate("");

  interfaceDFTB3_->statesHandler().store(Utils::StateSize::minimal);
  auto savedState = interfaceDFTB3_->statesHandler().popNewestState();
  auto& stateDensity = savedState->getMatrixState("Density Matrix");
  auto& actualDensity = underlyingDFTB3_.getDensityMatrix().restrictedMatrix();
...
TEST_F(ADFTBStatesHandlerTest, CanSaveRegularlySizedStateWithDFTB3) {
  underlyingDFTB3_.calculate(Utils::derivativeType::none);
  auto results = interfaceDFTB3_->calculate("");

  interfaceDFTB3_->statesHandler().store(Utils::StateSize::regular);
  auto savedState = interfaceDFTB3_->statesHandler().popNewestState();
  const auto& stateFock = savedState->getMatrixState("Fock Matrix");
  const auto& actualFock = underlyingDFTB3_.getFockMatrix().restrictedMatrix();
  const auto& stateDensity = savedState->getMatrixState("Density Matrix");
  const auto& actualDensity = underlyingDFTB3_.getDensityMatrix().restrictedMatrix();
...
TEST_F(ADFTBStatesHandlerTest, LoadsStateCorrectlyWithDFTB3) {
  ...
  SparrowState state(Utils::StateSize::minimal, dynamic_cast<GenericMethodWrapper&>(*interfaceDFTB3_));
  state.generateDensityMatrixState(densityMatrix, false);

  interfaceDFTB3_->statesHandler().load(std::make_shared<SparrowState>(std::move(state)));
  interfaceDFTB3_->statesHandler().store(Utils::StateSize::minimal);

  const auto loadedDensityMatrix = interfaceDFTB3_->statesHandler().popNewestState()->getMatrixState("Density Matrix");
...
TEST_F(ADFTBStatesHandlerTest, ReinitializesStateCorrectlyWithDFTB3) {
  auto& handler = interfaceDFTB3_->statesHandler();
